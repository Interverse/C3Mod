...
namespace C3Mod.GameTypes {
    internal class Apocalypse {
        ...
        public static void OnUpdate(EventArgs args) {
                    ...
                    if (hasEnoughPlayers()) {
                        if (StartCount == 0) {
                            ...
                            switch (r.Next(20) + 1) {
                                ...
                                case 11: {
                                    C3Tools.BroadcastMessageToGametype("apoc", "Johuan is awesome! Wave starting in: " + (StartCount * 5).ToString() + "...", Color.Cyan);
                                    break;
                                }
                                case 12: {
                                ...
                                case 16: {
                                    C3Tools.BroadcastMessageToGametype("apoc", "Redigit spites you! " + (StartCount * 5).ToString() + "...", Color.Cyan);
                                    break;
                                }
                                case 17: {
                            ...
                            C3Tools.BroadcastMessageToGametype("apoc", "Next Monster: " + CurMonster.FullName, Color.Cyan);
                            countDownTick = DateTime.UtcNow;
                            StartCount--;
                        }
                    } else {
            ...
            if (Running) {
                ...
                if (tick > (C3Mod.C3Config.MonsterApocalypseScoreNotifyInterval * 1000)) {
                    C3Tools.BroadcastMessageToGametype("apoc", CurMonster.FullName + "'s Left: " + MonstersLeft.ToString(), Color.Cyan);
                ...
                lock (C3Mod.C3Players) {
                    foreach (C3Player player in C3Mod.C3Players) {
                        if (player.GameType == "apoc") {
                            ...
                            NetMessage.SendData((int)PacketTypes.TogglePvp, -1, -1, null, player.TSPlayer.Index);
                            ...
                            if (Main.player[player.Index].dead) {
                                ...
                                if (player.LivesUsed >= C3Mod.C3Config.MonsterApocalypseLivesPerWave && !player.Dead) {
                                    ...
                                    if (playersdead >= apocplayers) {
                                        ...
                                        C3Tools.BroadcastMessageToGametype("apoc", "Your team survived the Apocalypse!", Color.Cyan);
                                        C3Tools.BroadcastMessageToGametype("", "Players on this server survived the apocalypse!", Color.Cyan);
                                        TpToSpawns(false);
                                        C3Tools.ResetGameType("apoc");
                                        Running = false;
                                        Intermission = false;
        ...
        public static void SpawnMonsters() {
            ...
            if (CurMonster.boss || CurMonster.FullName == "Betsy" || CurMonster.FullName == "Flying Dutchman") {
                var npc = new NPC();
                Random r = new Random();
                int amount = r.Next(3) + 1;

                for (int i = 0; i < amount; i++) {
                    int npcid = NPC.NewNPC((int)(MonsterSpawn.X * 16) - r.Next(-Radius, Radius) * 16,
                        (int)MonsterSpawn.Y * 16 - r.Next(-Radius, Radius) * 16, CurMonster.type);
                    Main.npc[npcid].SetDefaults(CurMonster.netID);
                    Main.npc[npcid].life = Main.npc[npcid].life * (Wave / 2) + 1;
        ...
        public static void TpToSpawns(bool pvpstate) {
            for (int i = 0; i < C3Mod.C3Players.Count; i++) {
                if (C3Mod.C3Players[i].GameType == "apoc") {
                    ...
                    NetMessage.SendData(30, -1, -1, null, C3Mod.C3Players[i].Index, 0f, 0f, 0f);
